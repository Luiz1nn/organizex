...
def create_parser() -> argparse.ArgumentParser:
    ...
    if input_ext != '.xlsx':
        raise ValueError(f"O arquivo de entrada deve ser um arquivo .xlsx (recebido: {args.input})")

    if output_ext != '.xls':
        raise ValueError(f"O arquivo de saída deve ser um arquivo .xls (recebido: {args.output})")
